...
class CornersProblem(search.SearchProblem):
  ...
  def isGoalState(self, state):
    ...
    visitedCorners_ = self.visitedCorners

    if node in self.corners:
        if not node in visitedCorners_:
          print 'TI SI GOVNO', visitedCorners_
          visitedCorners_.append(node)
        
        return len(visitedCorners_) == 4
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
        ...
        if not hitsWall:
            successorVisitedCorners = list(self.visitedCorners)
